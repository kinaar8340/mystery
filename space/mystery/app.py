...
def _nav_theme_gradio_css_vars() -> str:
    ...
    --myst-page-fill-height: 100%;
    --myst-status-chrome-height: {chrome};
    --myst-status-content-max-height: var(--myst-page-fill-height, calc(100dvh - {mc["status_viewport_offset"]}));
    --myst-render-detail-chrome-height: {detail_chrome};
    --myst-render-detail-max-height: var(--myst-page-fill-height, calc(100dvh - {detail_chrome}));
...
def _demo_viewport_preserve_active_demo(
    ...
    *,
    ...
    """Viewport stack — latched Demo B/C/D/E–I/F video; else D* geodesic; else Demo A startup."""
    letter = _normalize_active_demo_letter(active_demo_letter)
    ...
    if letter in _PRESET_DEMO_LETTERS:
        try:
            return _demo_viewport_show_preset_video(letter)
        except Exception:
            logger.exception("preset video preserve failed on dimension switch")
            return _demo_viewport_show_plot(dim_fig)
...
(function() {{
    ...
    function mystSyncGravityViewportHeight() {{
        ...
        var h = Math.max(360, Math.round(window.innerHeight - rect.top - 2));
    ...
    function mystActivePageEl() {{
        var sels = ['.myst-gravity-page', '.myst-readme-page', '.myst-render-page', '.myst-status-page'];
        for (var i = 0; i < sels.length; i++) {{
            if (mystPageVisible(sels[i])) return document.querySelector(sels[i]);
        }}
        return null;
    }}
    function mystClearActivePageFill(page) {{
        page = page || mystActivePageEl();
        if (!page) {{
            document.documentElement.style.removeProperty('--myst-page-fill-height');
            delete document.documentElement.dataset.mystPageFillH;
            return;
        }}
        delete page.dataset.mystPageFillH;
        page.style.removeProperty('min-height');
        page.style.removeProperty('height');
        page.style.removeProperty('max-height');
        var readmeScroll = page.querySelector('#readme-scroll-container');
        if (readmeScroll) {{
            readmeScroll.style.removeProperty('height');
            readmeScroll.style.removeProperty('max-height');
            readmeScroll.style.removeProperty('flex');
        }}
    }}
    function mystSyncActivePageFill() {{
        var page = mystActivePageEl();
        if (!page) {{
            mystClearActivePageFill();
            return;
        }}
        var rect = page.getBoundingClientRect();
        if (rect.width <= 0 || rect.top >= window.innerHeight) return;
        var h = Math.max(120, Math.round(window.innerHeight - rect.top - 2));
        var last = parseInt(page.dataset.mystPageFillH || '-1', 10);
        if (last === h) return;
        page.dataset.mystPageFillH = String(h);
        window.__mystViewportSyncing = true;
        try {{
            document.documentElement.style.setProperty('--myst-page-fill-height', h + 'px');
            page.style.setProperty('flex', '1 1 0', 'important');
            ...
            var readmeScroll = page.querySelector('#readme-scroll-container');
            if (readmeScroll) {{
                readmeScroll.style.setProperty('flex', '1 1 0', 'important');
                readmeScroll.style.setProperty('min-height', '0', 'important');
                readmeScroll.style.setProperty('height', '100%', 'important');
                readmeScroll.style.setProperty('max-height', '100%', 'important');
            }}
        }} finally {{
            window.__mystViewportSyncing = false;
        }}
    ...
    window.mystSyncActivePageFill = mystSyncActivePageFill;
...
(function() {{
    ...
    function bootRenderGridClicks() {{
        ...
        if (window.mystSyncActivePageFill) window.mystSyncActivePageFill();
...
html:has(.gradio-container .myst-status-page:not(.hide):not(.hidden)) {{
    overflow: hidden !important;
    max-height: 100dvh !important;
    height: 100dvh !important;
}}
#root:has(.gradio-container .myst-gravity-page:not(.hide):not(.hidden)),
#root:has(.gradio-container .myst-readme-page:not(.hide):not(.hidden)),
#root:has(.gradio-container .myst-render-page:not(.hide):not(.hidden)),
#root:has(.gradio-container .myst-status-page:not(.hide):not(.hidden)),
.app:has(.gradio-container .myst-gravity-page:not(.hide):not(.hidden)),
.app:has(.gradio-container .myst-readme-page:not(.hide):not(.hidden)),
.app:has(.gradio-container .myst-render-page:not(.hide):not(.hidden)),
.app:has(.gradio-container .myst-status-page:not(.hide):not(.hidden)) {{
...
body:has(.gradio-container .myst-status-page:not(.hide):not(.hidden)) {{
    overflow: hidden !important;
    max-height: 100dvh !important;
    height: 100dvh !important;
}}
...
.gradio-container:has(.myst-status-page:not(.hide):not(.hidden)) {{
    overflow: hidden !important;
    height: 100dvh !important;
...
.gradio-container:has(.myst-gravity-page:not(.hide):not(.hidden)) .myst-unified-nav-host,
.gradio-container:has(.myst-readme-page:not(.hide):not(.hidden)) .myst-unified-nav-host,
.gradio-container:has(.myst-render-page:not(.hide):not(.hidden)) .myst-unified-nav-host,
...
.gradio-container .myst-gravity-page {{
    --myst-gravity-viewport-height: var(--myst-page-fill-height, calc(100dvh - 11.5rem));
...
.gradio-container .myst-render-page {{
    ...
    --myst-render-grid-height: var(--myst-page-fill-height, calc(100dvh - 12.5rem));
...
.gradio-container .myst-render-page .grid-container,
.gradio-container .myst-render-page .myst-render-catalog-host.grid-container {{
    flex: 1 1 auto !important;
    min-height: 0 !important;
    height: 100% !important;
    max-height: none !important;
...
.gradio-container .myst-render-page .preset-grid,
.gradio-container .myst-render-page .preset-grid .html-container {{
    width: 100% !important;
    max-width: 100% !important;
    min-height: 0 !important;
    height: 100% !important;
...
.gradio-container .myst-render-page .myst-render-stack > .block:has(.myst-render-action-row),
.gradio-container .myst-render-page .myst-render-stack > .form:has(.myst-render-action-row) {{
    flex: 0 0 auto !important;
}}
...
_BUNDLED_DEMO_DEFORM_VIDEOS: dict[str, str] = {
    ...
    "H": os.path.join(_APP_DIR, "assets", "demo_h_d12_dodecahedron.mp4"),
    "I": os.path.join(_APP_DIR, "assets", "demo_i_d20_icosahedron.mp4"),
}
...
def _create_breathing_animation(*, fresh: bool = False):
    """Looping breathing Plotly figure for Demo F — ready to animate on load."""
...
def _get_deform_demo_video_path(letter: str) -> str:
    ...
    if active not in _DEMO_DEFORM_VIDEO_CACHE:
        ...
        if bundled and os.path.isfile(bundled):
            ...
            dimension = _DEFORM_DEMO_SHAPE_BY_LETTER.get(active, "D4")
            raw_path = render_platonic_deformation_demo_video(
                dimension, n_per_segment=8, fps=10, dpi=80
            )
...
def _get_preset_demo_video_path(letter: str) -> str:
    ...
    if active not in _DEMO_PRESET_VIDEO_CACHE:
        ...
        if bundled and os.path.isfile(bundled):
            ...
            pressure = _PRESET_DEMO_PRESSURE_BY_LETTER.get(active, 0.50)
            raw_path = render_d6_convex_preset_demo_video(
                pressure, n_per_segment=8, fps=10, dpi=80
            )
        _DEMO_PRESET_VIDEO_CACHE[active] = _cache_media_for_gradio(raw_path)
    return _DEMO_PRESET_VIDEO_CACHE[active]
...
def _get_breathing_demo_video_path() -> str:
    ...
    if _BREATHING_DEMO_VIDEO_CACHE is None:
        if os.path.isfile(_BUNDLED_BREATHING_VIDEO_MP4):
            raw_path = _BUNDLED_BREATHING_VIDEO_MP4
            print(f"[breathing] using bundled mp4: {raw_path}", flush=True)
        elif os.path.isfile(_BUNDLED_BREATHING_VIDEO):
            raw_path = _BUNDLED_BREATHING_VIDEO
            print(f"[breathing] using bundled gif: {raw_path}", flush=True)
        else:
...
def _get_rigid_preset_plotly_figure(*, active_shape: str = _NO_ACTIVE_SHAPE):
    """Preset 05 rigid geodesic — fast placeholder while demo videos encode."""
...
def _deform_demo_fallback_figure(letter: str):
    """Rigid geodesic Plotly fallback while a deform demo MP4 encodes."""
    active = str(letter or "E").strip().upper()
    shape = _DEFORM_DEMO_SHAPE_BY_LETTER.get(active, "D4")
...
def _switch_gravity_demo_viewport_only(letter: str) -> tuple:
    ...
    if letter in _PRESET_DEMO_LETTERS:
        return _demo_preset_viewport_only(letter)
    if letter in _DEFORM_DEMO_LETTERS:
...
def _demo_preset_viewport_only(letter: str) -> tuple:
    ...
    try:
        ...
        logger.exception("preset demo video failed for Demo %s", active)
        return _demo_viewport_show_plot(_get_rigid_preset_plotly_figure())


def _demo_deform_viewport_only(letter: str) -> tuple:
    """Demo E/G/H/I viewport — platonic deformation MP4 after tab latch."""
    active = str(letter or "E").strip().upper()
    if active not in _DEFORM_DEMO_LETTERS:
        active = "E"
    try:
        ...
        return _demo_viewport_show_plot(_deform_demo_fallback_figure(active))
...
def _launch_deform_demo(letter: str) -> tuple:
    """Demo E/G/H/I — latch + platonic deformation loop (single-shot fallback for programmatic calls)."""
    active = str(letter or "E").strip().upper()
    if active not in _DEFORM_DEMO_LETTERS:
        active = "E"
...
def _switch_gravity_demo(letter: str) -> tuple:
    """Switch the Home viewport to the demo for the selected letter (A, B–E, G–I)."""
    letter = str(letter).strip().upper()
    if letter == "A":
    ...
    if letter in _PRESET_DEMO_LETTERS:
        return _launch_preset_demo(letter)
    if letter in _DEFORM_DEMO_LETTERS:
...
def build_app() -> gr.Blocks:
    ...
    with gr.Blocks(
        ...
        with gr.Column(visible=False, elem_classes=["myst-render-page"], scale=1) as page_render:
            with gr.Column(visible=True, elem_classes=["myst-render-stack"], scale=1) as render_content_col:
                ...
                with gr.Column(
                    visible=True,
                    scale=1,
                    elem_classes=["myst-render-catalog-host", "grid-container"],
        ...
        def _app_boot() -> tuple:
            ...
            try:
                ...
                logger.exception("app boot startup image failed")
